...
export const slice = createSlice({
    ...
    initialState,
    reducers: {
        pushHistoryStack: (state, {payload}: PayloadAction<{ historyKey: string }>) => {
        ...
        popHistoryStack: (state, {payload}: PayloadAction<{ historyKey: string }>) => {
            ...
            const isPopEvent = state.historyStack.indexOf(payload.historyKey) !== -1;
            if(isPopEvent) {
                // 履歴の最後の要素を削除する
                state.historyStack.pop();
